#Uses this source: https://data-hrm.hub.arcgis.com/datasets/HRM::by-law-areas/about
...
def create_triples(input_geojson: str, output_file: str, input_csv:str = '', format: str ="turtle", sample: bool = False):
    ...
    for index, feature in enumerate(tqdm(data["features"])): #Building parcel triples
        ...
        try:
            obj_id = curr_row.get("OBJECTID")
            bylaw_uri = URIRef(COT[f"BylawArea{obj_id}"])
            g.add((bylaw_uri, RDF.type, HP.ZoningType))

            if curr_row["BYLAW_NAME"]:
                g.add((bylaw_uri, GENPROP.hasName, Literal(curr_row["BYLAW_NAME"])))
            ...
            try:
                ...
                loc_uri = URIRef(COT[f"BylawAreaLoc{obj_id}"])
                g.add((loc_uri, RDF.type, LOC.Location))
                g.add((bylaw_uri, LOC.hasLocation, loc_uri))
                g.add((
                    loc_uri,
                    GEO.asWKT,
...
if __name__ == "__main__":
    create_triples(input_geojson="raw_data/bylaw_areas.geojson",
                        #   input_csv="raw_data/building_polygons.csv",
                          output_file="sample_triples/bylaw_areas.ttl",
                          sample= True,
                          format="ttl")